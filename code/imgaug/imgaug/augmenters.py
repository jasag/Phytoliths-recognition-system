...
class Augmenter(object):
    ...
    '''Devuelve las imagenes por separado en lugar de una unica imagen con todas ellas'''
    # TODO
    # Sean cambiado unos pocos detalles del metodo anterior unicamente.
    # Por lo tanto es mejorable la reutilizacion
    def draw_grid_v2(self, images, rows, cols):
        if ia.is_np_array(images):
            if len(images.shape) == 4:
                images = [images[i] for i in range(images.shape[0])]
            elif len(images.shape) == 3:
        ...
        for row_idx in range(rows):
            for img_idx, image in enumerate(images):
                for col_idx in range(cols):
                    ...
                    cell_x1 = cell_width * col_idx
                    cell_x2 = cell_x1 + image_aug.shape[1]
                    grid.append(np.array(image_aug))
        return grid
